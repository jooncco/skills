...
All RE artifacts are created under `codekb/<repo>/` — the durable per-repo code
knowledge base shared across runs (the directory `codekb.ts path --repo <repo>`
resolves). Run-scoped working files live under `codekb/.record/<repo>/`.

### Required Artifacts

1. **business-overview.md** — Business domain context, purpose, key functionality
2. **architecture.md** — System architecture, patterns, component relationships, Mermaid diagrams (MUST include `## Interaction Diagrams`)
...
9. **reverse-engineering-timestamp.md** - Records when reverse engineering was performed (date, commit hash if available) plus the structured Scope of Analysis block (template below). The scope block is machine-read by `codekb.ts scope-diff` on the next rerun, so its accuracy decides whether a future run can reuse the verified coverage or must merge/replace it.

Section skeletons for artifacts 1-8 live in `~/.config/opencode/skills/reverse-engineering/templates/<artifact>.md`.
Their `##` headings are the expected heading set the `codekb.ts sensor` check
verifies at the gate (advisory). Fill the skeleton; add sections freely.

### Developer Code Scan Template

Written by the developer (link 1) to `codekb/.record/<repo>/developer-scan.md`.
...
fingerprint: [output of `codekb.ts scope-diff --mint --paths <analyzed paths>` - verbatim; it prints "unknown" when not computable]
analyzed:
  paths:
...
- Build all 9 candidate artifacts under the temporary `codekb/.record/<repo>/.stage/` directory. Never write a cumulative merge directly into the shared CodeKB.
- The pre-scan `codekb.ts snapshot` paths bound verified coverage. If the scan discovers a deep path outside that set, take a new snapshot and repeat the scan over the expanded set.
- Publish only through `codekb.ts publish` with the snapshot's store generation and source fingerprint. A store-generation conflict requires re-reading and re-merging the winner's store; a source conflict requires a fresh scan.
