...
Markdown file under `~/.config/opencode/skills/reverse-engineering/knowledge/architect/` (`re-synthesis-checklist.md`,
`architecture-patterns.md`) AND `~/.config/opencode/skills/reverse-engineering/knowledge/developer/re-artifacts.md`
(the artifact templates and the Scope of Analysis rules you must follow), plus
`~/.config/opencode/skills/reverse-engineering/knowledge/shared/` if present. Read the `~/.config/opencode/skills/reverse-engineering/templates/`
...
- NEVER write into `codekb/<repo>/` directly. Publication goes through
  `bun ~/.config/opencode/skills/reverse-engineering/scripts/codekb.ts publish ...` with the snapshot values from
