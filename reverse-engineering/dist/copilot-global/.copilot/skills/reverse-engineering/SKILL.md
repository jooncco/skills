...
- `~/.copilot/skills/reverse-engineering` — this skill's installed directory, project-relative
...
- `codekb.ts` — `bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts` (run from `<project>`).
  Run `bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts help` for the verb list.
...
```
bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts attempt start --repo <repo> --intent <intent>
```
...
```
bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts scope-diff --repo <repo>
```
...
```
bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts reuse --repo <repo>
```
...
```
bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts snapshot --repo <repo> --paths <source paths> --json
```
...
Scan Template in `~/.copilot/skills/reverse-engineering/knowledge/developer/re-artifacts.md`.
...
```
bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts link --repo <repo> --link developer --artifact codekb/.record/<repo>/developer-scan.md
```
...
- The store directory printed by `bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts path --repo <repo>`
...
- The exact tool lines the architect must run, in order:
  1. mint: `bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts scope-diff --repo <repo> --mint --paths <final analyzed paths>` → paste output verbatim into the scope block's `fingerprint:`
  2. (only when Step 1 found an existing store) write the timestamp content to
     `codekb/.record/<repo>/scope-draft.md`, run
     `bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts scope-diff --repo <repo> --compare codekb/.record/<repo>/scope-draft.md`
  ...
  3. publish:
     ```
     bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts publish --repo <repo> \
...
```
bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts link --repo <repo> --link architect
```

Do not proceed until every selected repo's chain has both receipts
(`bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts attempt status --repo <repo>` shows
...
```
bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts sensor --dir codekb/<repo>/
```
...
`bun ~/.copilot/skills/reverse-engineering/scripts/codekb.ts attempt status --repo <repo> --json` for each
