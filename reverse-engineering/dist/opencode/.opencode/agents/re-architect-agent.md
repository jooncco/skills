...
Markdown file under `.opencode/skills/reverse-engineering/knowledge/architect/` (`re-synthesis-checklist.md`,
`architecture-patterns.md`) AND `.opencode/skills/reverse-engineering/knowledge/developer/re-artifacts.md`
(the artifact templates and the Scope of Analysis rules you must follow), plus
`.opencode/skills/reverse-engineering/knowledge/shared/` if present. Read the `.opencode/skills/reverse-engineering/templates/`
...
- NEVER write into `codekb/<repo>/` directly. Publication goes through
  `bun .opencode/skills/reverse-engineering/scripts/codekb.ts publish ...` with the snapshot values from
