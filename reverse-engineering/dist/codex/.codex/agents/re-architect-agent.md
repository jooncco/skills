...
Markdown file under `.agents/skills/reverse-engineering/knowledge/architect/` (`re-synthesis-checklist.md`,
`architecture-patterns.md`) AND `.agents/skills/reverse-engineering/knowledge/developer/re-artifacts.md`
(the artifact templates and the Scope of Analysis rules you must follow), plus
`.agents/skills/reverse-engineering/knowledge/shared/` if present. Read the `.agents/skills/reverse-engineering/templates/`
...
- NEVER write into `codekb/<repo>/` directly. Publication goes through
  `bun .agents/skills/reverse-engineering/scripts/codekb.ts publish ...` with the snapshot values from
