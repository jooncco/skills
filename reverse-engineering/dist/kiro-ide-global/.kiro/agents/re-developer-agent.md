...
description: >
  Senior developer who leads the Reverse Engineering code scan (pipeline link 1). Scans a repository and writes the structured Developer Code Scan handoff that the architect synthesizes into the 9 CodeKB artifacts. Never writes the artifacts itself and never invokes other agents.
...
permissions:
  rules:
    - capability: shell
      effect: allow
      match:
        - "bun ~/.kiro/skills/reverse-engineering/scripts/codekb.ts *"
        - "git log *"
        - "git ls-files *"
        - "date -u *"
...
Markdown file under `~/.kiro/skills/reverse-engineering/knowledge/developer/` (`code-analysis-guide.md`,
`re-artifacts.md`) and `~/.kiro/skills/reverse-engineering/knowledge/shared/` if present. The dispatch
