...
Markdown file under `.cursor/skills/reverse-engineering/knowledge/architect/` (`re-synthesis-checklist.md`,
`architecture-patterns.md`) AND `.cursor/skills/reverse-engineering/knowledge/developer/re-artifacts.md`
(the artifact templates and the Scope of Analysis rules you must follow), plus
`.cursor/skills/reverse-engineering/knowledge/shared/` if present. Read the `.cursor/skills/reverse-engineering/templates/`
...
- NEVER write into `codekb/<repo>/` directly. Publication goes through
  `bun .cursor/skills/reverse-engineering/scripts/codekb.ts publish ...` with the snapshot values from
