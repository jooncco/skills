# Dispatch — Cursor annex

How the conductor delegates a pipeline link on this harness.

## Native subagents

The persona files in `.cursor/agents/` (`re-developer-agent.md`,
`re-architect-agent.md`) ARE live Cursor subagents — Cursor discovers them by
frontmatter `name`. Delegate with the **`task`** tool targeting the agent by
