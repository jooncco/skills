...
- `~/.codex/skills/reverse-engineering` — this skill's installed directory, project-relative
...
- `codekb.ts` — `bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts` (run from `<project>`).
  Run `bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts help` for the verb list.
...
```
bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts attempt start --repo <repo> --intent <intent>
```
...
```
bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts scope-diff --repo <repo>
```
...
```
bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts reuse --repo <repo>
```
...
```
bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts snapshot --repo <repo> --paths <source paths> --json
```
...
Scan Template in `~/.codex/skills/reverse-engineering/knowledge/developer/re-artifacts.md`.
...
```
bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts link --repo <repo> --link developer --artifact codekb/.record/<repo>/developer-scan.md
```
...
- The store directory printed by `bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts path --repo <repo>`
...
- The exact tool lines the architect must run, in order:
  1. mint: `bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts scope-diff --repo <repo> --mint --paths <final analyzed paths>` → paste output verbatim into the scope block's `fingerprint:`
  2. (only when Step 1 found an existing store) write the timestamp content to
     `codekb/.record/<repo>/scope-draft.md`, run
     `bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts scope-diff --repo <repo> --compare codekb/.record/<repo>/scope-draft.md`
  ...
  3. publish:
     ```
     bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts publish --repo <repo> \
...
```
bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts link --repo <repo> --link architect
```

Do not proceed until every selected repo's chain has both receipts
(`bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts attempt status --repo <repo>` shows
...
```
bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts sensor --dir codekb/<repo>/
```
...
`bun ~/.codex/skills/reverse-engineering/scripts/codekb.ts attempt status --repo <repo> --json` for each
