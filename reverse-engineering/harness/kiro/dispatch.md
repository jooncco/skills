# Dispatch — Kiro CLI annex

How the conductor delegates a pipeline link on this harness.

## Native crew dispatch

The personas ship as Kiro custom agents in `{{AGENTS_DIR}}/`
(`re-developer-agent.json` + `.md`, `re-architect-agent.json` + `.md`).
...
- The agent's `resources` load its persona and knowledge; do NOT paste persona
...
`{{AGENTS_DIR}}/re-developer-agent.md` (or `re-architect-agent.md`) and every
