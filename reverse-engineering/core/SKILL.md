...
- `{{SKILL_DIR}}` — this skill's installed directory, project-relative
...
- `codekb.ts` — `bun {{SKILL_DIR}}/scripts/codekb.ts` (run from `<project>`).
  Run `bun {{SKILL_DIR}}/scripts/codekb.ts help` for the verb list.
...
```
bun {{SKILL_DIR}}/scripts/codekb.ts attempt start --repo <repo> --intent <intent>
```
...
```
bun {{SKILL_DIR}}/scripts/codekb.ts scope-diff --repo <repo>
```
...
```
bun {{SKILL_DIR}}/scripts/codekb.ts reuse --repo <repo>
```
...
```
bun {{SKILL_DIR}}/scripts/codekb.ts snapshot --repo <repo> --paths <source paths> --json
```
...
Scan Template in `{{SKILL_DIR}}/knowledge/developer/re-artifacts.md`.
...
```
bun {{SKILL_DIR}}/scripts/codekb.ts link --repo <repo> --link developer --artifact codekb/.record/<repo>/developer-scan.md
```
...
- The store directory printed by `bun {{SKILL_DIR}}/scripts/codekb.ts path --repo <repo>`
...
- The exact tool lines the architect must run, in order:
  1. mint: `bun {{SKILL_DIR}}/scripts/codekb.ts scope-diff --repo <repo> --mint --paths <final analyzed paths>` → paste output verbatim into the scope block's `fingerprint:`
  2. (only when Step 1 found an existing store) write the timestamp content to
     `codekb/.record/<repo>/scope-draft.md`, run
     `bun {{SKILL_DIR}}/scripts/codekb.ts scope-diff --repo <repo> --compare codekb/.record/<repo>/scope-draft.md`
  ...
  3. publish:
     ```
     bun {{SKILL_DIR}}/scripts/codekb.ts publish --repo <repo> \
...
```
bun {{SKILL_DIR}}/scripts/codekb.ts link --repo <repo> --link architect
```

Do not proceed until every selected repo's chain has both receipts
(`bun {{SKILL_DIR}}/scripts/codekb.ts attempt status --repo <repo>` shows
...
```
bun {{SKILL_DIR}}/scripts/codekb.ts sensor --dir codekb/<repo>/
```
...
`bun {{SKILL_DIR}}/scripts/codekb.ts attempt status --repo <repo> --json` for each
